...
impl GaussSeidel {
    fn calcular(&mut self) {
        let mut valores = vec![0.0; self.valores.len()];
        for _ in 0..20 {
            for (index, renglon) in self.valores.iter().enumerate() {
                let len = renglon.len();
                let mut val = self.resultados[index];

                for index2 in 0..len {
                    if index == index2 {
                        continue;
                    }
                    val -= self.valores[index][index2] * valores[index2];
                }

                valores[index] = val / self.valores[index][index];
            }
        }
        self.incognitas = valores;
    }
}
...
impl Lagrance {
    fn calcular(&self, x: f64) -> f64 {
        let mut tot = 0.;
        for (i, val) in self.valores.iter().enumerate() {
            let mut a = 1.;
            let mut b = 1.;
            for (j, val2) in self.valores.iter().enumerate() {
                if j == i {
                    continue;
                }
                a *= x - val2.0;
                b *= self.valores[i].0 - val2.0;
            }
            let div = a / b;
            tot += div * val.1;
        }
...
impl Default for Proyecto1 {
    fn default() -> Self {
        ...
        Self {
            ...
            metodo_lagrance: Lagrance { valores: v },
            metodo_gauss: GaussSeidel {
                valores: g,
                resultados: r,
                ..Default::default()
...
impl Funcion {
    ...
    fn to_derivada(&self) -> Funcion {
        ...
        fun.ecuaciones[len - 1].positivo =
            fun.ecuaciones[len - 1].multiplicacion * fun.ecuaciones[len - 1].positivo;
...
fn compilar_funcion(funcion: &str, vec: &mut Funcion) {
    ...
    loop {
        ...
        if ultimo_valor(funcion, indice) {
            // +1x^3-6x^2+11x^1-6
            if elemento == '+' {
...
fn func_to_gui(s: &str) -> LayoutJob {
    ...
    match s.find('^') {
        ...
        None => job.append(s, 0.0, TextFormat::default()),
    }
    job
}
...
impl eframe::App for Proyecto1 {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading("Calculadora Metodos Numericos"); // Titulo
    ...
    Con un valor en Y de: {}",
            ...
            Plot::new("Plot")
                .view_aspect(0.1)
                .show(ui, |plot_ui| plot_ui.line(line)); // Se
                                                         // muestra el grafico
        });
...
impl DivisionSintetica {
    fn obtener_resultados(&mut self) {
        ...
        for factor in &self.factores {
            // Por cada factor
            self.resultados
                .push(division_sin(factor, &self.terminos_in)) // Obtenemos el
    ...
    fn actualizar_datos(&mut self, datos: &Funcion) {
        ...
        for fac in term_in_factors {
            for max_fac in &max_factors {
                let val = fac % max_fac;
                if val == 0 {
                    self.factores.push(fac / max_fac)
                }
            }
        }
        let h: Vec<i32> = datos
...
fn get_factors(n: i32) -> Vec<i32> {
    (1..n + 1)
        .into_iter()
        .filter(|&x| n % x == 0)
        .collect::<Vec<i32>>() // En un rango de 1 hasta
...
impl MetodoBiseccion {
    ...
    fn buscar_raiz(&mut self, funcion: &Funcion, num_raices: i32) {
        // Busca dos numeros de a y b tal que f(a)*f(b)<0
        let mut min = -5.;
        let paso = 0.33454;
        let max_iter = 1000;
